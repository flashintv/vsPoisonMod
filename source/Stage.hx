			...
			case 'plains':
				{
					...
					var bg = new FlxSprite(0, 0).loadGraphic(Paths.image('plainBG', 'week1'));
					bg.scrollFactor.set(0.9, 0.95);
					bg.screenCenter();
					bg.y += 100;
					...
					var iceTruck = new FlxSprite(-200, -900).loadGraphic(Paths.image('twotrucks', 'week1'));
					iceTruck.scale.set(0.3, 0.3);
					iceTruck.scrollFactor.set(0.85, 0.95);
					iceTruck.antialiasing = FlxG.save.data.antialiasing;
					swagBacks['iceTruck'] = iceTruck;
					toAdd.push(iceTruck);
				}
			default:
				{

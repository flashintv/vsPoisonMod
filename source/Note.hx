...
class Note extends FlxSprite
{
	...
	public var notePoison:Bool = false;
	...
	public function new(strumTime:Float, noteData:Int, ?prevNote:Note, ?sustainNote:Bool = false, ?inCharter:Bool = false, ?isAlt:Bool = false,
			?bet:Float = 0, ?poisonNote:Bool = false)
	{
		super();
		...
		if (notePoison)
		{
			frames = Paths.getSparrowAtlas('arrows/poisonArrows', 'shared');

			animation.addByPrefix('greenScroll', 'greenScroll');
			animation.addByPrefix('redScroll', 'redScroll');
			animation.addByPrefix('purpleScroll', 'purpleScroll');
			animation.addByPrefix('blueScroll', 'blueScroll');

			setGraphicSize(Std.int(width * 0.2425));
			updateHitbox();
			antialiasing = FlxG.save.data.antialiasing;
		}
		else
		{
			if (inCharter) // i dont really give a flying fuck about the inCharter shit so
			{
				frames = Paths.getSparrowAtlas('arrows/NOTE_assets');

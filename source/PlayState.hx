...
class PlayState extends MusicBeatState
{
	...
	// lua but in code pog fuck lua tbh i hate that shit
	var luaCode:Bool = false;
	var showOnlyStrums:Bool = false;
	var strumLine1Visible:Bool = false;
	var strumLine2Visible:Bool = false;
	// i wont probably use this anyway ¯\_(ツ)_/¯
	...
	// poison shit
	var poisonTimer:FlxTimer = new FlxTimer();
	var poisonEffect:Bool = false;
	...
	public function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				var daOriginalTypeNotStolenImWritingThisFromMyMemory:Bool = songNotes[5];
				...
				var swagNote:Note = new Note(daStrumTime, daNoteData, oldNote, false, false, false, songNotes[4],
					daOriginalTypeNotStolenImWritingThisFromMyMemory);
				...
				for (susNote in 0...Math.floor(susLength))
				{
					...
					var sustainNote:Note = new Note(daStrumTime + (Conductor.stepCrochet * susNote) + Conductor.stepCrochet, daNoteData, oldNote,
						true); // not adding poison arrows sustain cuz thats weirdchamp and i dont have assets for it
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			switch (noteTypeCheck)
			{
				...
				default:
					babyArrow.frames = Paths.getSparrowAtlas('arrows/NOTE_assets', 'shared');
					for (j in 0...4)
					{
	...
	override public function update(elapsed:Float)
	{
		...
		if (luaCode)
		{
			...
			if (strumLine1Visible || strumLine2Visible)
			{
				for (i in 0...4)
				{
					strumLineNotes.members[i].visible = strumLine1Visible;
					if (i <= playerStrums.length)
						playerStrums.members[i].visible = strumLine2Visible;
				}
			}
		}
						...
						else
						{
							...
							else
							{
									...
									&& daNote.spotInLine != daNote.parent.children.length)
								{
									...
									if (daNote.parent.wasGoodHit && !daNote.parent.notePoison)
										misses++;
									updateAccuracy();
								}
								else if (!daNote.wasGoodHit && !daNote.isSustainNote && !daNote.notePoison)
								{
									health -= 0.15;
								}
							}
			...
			if (pressArray.contains(true) && generatedMusic)
			{
				...
				if (boyfriend.holdTimer > Conductor.stepCrochet * 4 * 0.001 && (!holdArray.contains(true) || PlayStateChangeables.botPlay))
				{
					if (boyfriend.animation.curAnim.name.startsWith('sing')
						...
						&& !boyfriend.animation.curAnim.name.startsWith('scared') && !poisonEffect
	...
	function noteMiss(direction:Int = 1, daNote:Note):Void
	{
		if (daNote.notePoison)
		{
			...
			if (!loadRep && daNote.mustPress)
			{
				var array = [daNote.strumTime, daNote.sustainLength, daNote.noteData, noteDiff];
				if (daNote.isSustainNote)
					array[1] = -1;
				saveNotes.push(array);
				saveJudge.push(daNote.rating);
			}
	...
	function goodNoteHit(note:Note, resetMashViolation = true):Void
	{
		if (note.notePoison)
		{
			...
			{
				poisonEffect = true;
				poisonTimer.start(2, function(tmr:FlxTimer)
				{
					poisonEffect = false;
	...
	override function beatHit()
	{
		...
		if (!endingSong && currentSection != null)
		{
			...
			if (!boyfriend.animation.curAnim.name.startsWith("sing") && !boyfriend.animation.curAnim.name.startsWith('scared') && (curBeat % idleBeat == 0 || !idleToBeat))
			{
